...
function addStock() {
    inquirer.prompt([
        ...
        databaseAdd(itemSelected, countWanted);
    });
};
...
function databaseAdd(item, amount) {
    connection.query(`SELECT * FROM items WHERE id = '${item}'`, function (error, response) {
        if (error) { throw error }
            ...
            connection.query(`SELECT * FROM items WHERE id = '${item}'`, function (error, response) {
                console.log("Items have been added to our stock. There are now " + response[0].Stock + " " + response[0].ItemName + ".")
            });
        };
        connection.end();
    })
...
function addItem() {
    inquirer.prompt([
        ...
        console.log("New item has been added! Here is our updated store.")
        viewItems();
    });
}
